...
DOMAIN_ORDER = os.environ.get("DOMAIN_ORDER", "legal,grants,aviation,self_healing,medical,cre").split(",")

# ── SCALE CONFIG PER DOMAIN ──
# Rule: scale model must be >= the model that generated the pair.
# Don't let 7B judge a 27B pair. The scale must match the weight class.
LOCAL = "http://localhost:11434"
WHALE = "http://192.168.0.99:11434"

DOMAIN_SCALES = {
    # Standard domains (generated by smaller models) — 12B+ scales
    "grants":       {"a_endpoint": LOCAL, "a_model": "gemma3:12b", "b_endpoint": WHALE, "b_model": "qwen2.5:7b"},
    "legal":        {"a_endpoint": LOCAL, "a_model": "gemma3:12b", "b_endpoint": WHALE, "b_model": "qwen2.5:7b"},
    "medical":      {"a_endpoint": LOCAL, "a_model": "gemma3:12b", "b_endpoint": WHALE, "b_model": "qwen2.5:7b"},
    "cre":          {"a_endpoint": LOCAL, "a_model": "gemma3:12b", "b_endpoint": WHALE, "b_model": "qwen2.5:7b"},
    "aviation":     {"a_endpoint": LOCAL, "a_model": "gemma3:12b", "b_endpoint": WHALE, "b_model": "qwen2.5:7b"},
    "self_healing": {"a_endpoint": LOCAL, "a_model": "gemma3:12b", "b_endpoint": WHALE, "b_model": "qwen2.5:7b"},
    # Heavy domains (generated by 27B+ models) — 27B+ scales
    "clawhash":     {"a_endpoint": LOCAL, "a_model": "gemma4:31b",  "b_endpoint": LOCAL, "b_model": "qwen3.5:27b"},
    "agenthash":    {"a_endpoint": LOCAL, "a_model": "gemma4:31b",  "b_endpoint": LOCAL, "b_model": "qwen3.5:27b"},
}

# Fallback for domains not in the map (env vars or defaults)
DEFAULT_SCALE = {
    "a_endpoint": os.environ.get("JUDGE_A", LOCAL),
    "a_model": os.environ.get("JUDGE_A_MODEL", "gemma3:12b"),
    "b_endpoint": os.environ.get("JUDGE_B", WHALE),
    "b_model": os.environ.get("JUDGE_B_MODEL", "qwen2.5:7b"),
}


def get_scales(domain):
    """Get scale A/B config for a domain. Heavy domains get heavy scales."""
    cfg = DOMAIN_SCALES.get(domain, DEFAULT_SCALE)
    return cfg["a_endpoint"], cfg["a_model"], cfg["b_endpoint"], cfg["b_model"]
...
def classify(score):
    """Weight class: you have to earn Class A."""
    if score >= 0.85:
        return "royal_jelly"
    elif score >= 0.70:
        return "honey"
